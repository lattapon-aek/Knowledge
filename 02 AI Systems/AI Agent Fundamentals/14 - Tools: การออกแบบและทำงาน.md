...
ในเชิงสถาปัตย์ สิ่งสำคัญไม่ใช่ syntax helper ของ framework ใด framework หนึ่ง แต่คือ contract ระหว่าง model กับ runtime:
- tool name
- description
- argument schema
- execution boundary
...
ในเชิงสถาปัตย์ tools หรือ tool schemas ต้องถูกส่งเข้าไปใน runtime request/context เพื่อให้ model รู้จักก่อนเริ่มทำงาน OpenAI ใช้ `tools` parameter ใน Responses API ส่วน Anthropic ใช้ `tools` parameter และ content blocks สำหรับ `tool_use` / `tool_result`
