...
> เพิ่มจาก IBM AI Agent Governance

เมื่อ agents ทำงาน autonomous มากขึ้น monitoring ต้องขยายจาก tool/output level ไปสู่ **agent behavior level**:

### Governance Agents

จับคู่ working agents กับ "governance agents" ที่ทำหน้าที่ monitor และ evaluate agents อื่น:
- ตรวจว่า agent ทำตาม policy ไหม
- ป้องกัน potential harm ก่อนเกิด
- เหมือน hall monitor ที่ดึง agent ที่เริ่มผิดปกติออกมา

### Agent-to-Agent Monitoring

ใน multi-agent ecosystems ต้อง monitor interactions ระหว่าง agents:
- ตรวจว่า agents collaborate อย่างถูกต้อง
- conflict resolution rules สำหรับ agents ที่ขัดแย้งกัน
- ป้องกัน cascading failures จาก agent หนึ่งไปอีกตัว

### Behavioral Drift Detection

agents ที่ adapt จาก interactions อาจ drift ไปจาก intended behavior:
- customer service agent ที่รับ grumpy customers ทั้งวันอาจ develop bad-tempered personality — ในระบบที่มี persistent memory หรือ continuous fine-tuning loop ที่ feedback กลับเข้า model
- continuous monitoring ต้องตรวจ model drift ใน agent behavior ไม่ใช่แค่ model weights
- ต้องมี baseline behavior metrics เพื่อเปรียบเทียบ

### AI Sandboxing

ทดสอบ agents ใน simulated environments ก่อน deploy:
- ให้ agents ตัดสินใจโดยไม่มี real-world consequences
- ทดสอบ ethical dilemmas และ edge cases
- stress test ด้วย adversarial scenarios

### Emergency Shutdown

agents ใน high-risk environments ควรมี:
- emergency shutdown mechanism ที่ deactivate ได้ทันที
- containment procedures ป้องกัน malfunctioning agent escalate issues
- clear escalation path ไป human oversight
