...
ใน OpenAI API, vector stores เป็น infrastructure ร่วมของ 2 usage หลัก:
- semantic search ผ่าน Retrieval API / vector store search
- `file_search` tool ใน Responses API และ Assistants API

เมื่อเพิ่มไฟล์เข้า vector store ระบบจะ process ไฟล์เป็น searchable representation ผ่านการ chunk, embed, และ index
ตัว object สำคัญคือ:
- `file` = content ที่ upload ผ่าน Files API
- `vector_store` = container ของ searchable files
- `vector_store.file` = wrapper ของ file ที่ถูกผูกกับ vector store และมี attributes สำหรับ filtering

ผลเชิงสถาปัตย์:
- vector store ไม่ใช่แค่ storage แต่เป็น search index ที่มี lifecycle
- file attributes เป็น schema สำคัญของ retrieval control
- ถ้าจะใช้หลาย corpus ต้องตัดสินใจตั้งแต่ต้นว่าแยก vector stores หรือใช้ store เดียวพร้อม filters
...
สำหรับ OpenAI vector store files, attributes เป็น key-value metadata ที่นำไปใช้กับ filters ได้
ตัวอย่าง production attributes ที่ควรคิดตั้งแต่ ingestion:
- `tenant_id`
- `document_type`
- `source_system`
- `access_group`
...
ข้อควรระวัง:
- attributes ต้องเป็นส่วนหนึ่งของ indexing pipeline ไม่ใช่เติมทีหลังแบบ manual
- filters ช่วยจำกัดผลลัพธ์ แต่ไม่ควรถูกใช้แทน authorization layer
- ถ้า metadata ไม่ครบ retrieval จะมี blind spot แม้ embedding quality จะดี

---

## Search API Controls

OpenAI vector store search รองรับ controls หลักที่มีผลต่อคุณภาพและต้นทุน:
- `query` เป็น string หรือ array ของ query
- `filters` จำกัดผลตาม file attributes
- `max_num_results` จำกัดจำนวนผลลัพธ์
- `ranking_options` ปรับ ranking behavior
- `rewrite_query` ให้ระบบ rewrite query เพื่อ vector search

ผลต่อระบบ:
- `max_num_results` ต่ำช่วยลด latency และ context cost แต่เพิ่มความเสี่ยง missed evidence
- `filters` เพิ่ม precision และช่วย enforce scope
- `ranking_options` และ `rewrite_query` ต้องวัดด้วย eval เพราะอาจเปลี่ยน distribution ของผลลัพธ์
- search results ควรถูก log พร้อม scores, file origin, และ filters ที่ใช้ เพื่อ debug retrieval ได้
...
- ถ้าใช้ OpenAI file search ให้ treat vector store เป็น production index ที่ต้องมี schema, lifecycle, และ eval
- log retrieval controls ทุกครั้งที่ debug answer quality เช่น query, filters, `max_num_results`, และ ranking config
...
- [[02 AI Systems/RAG/Core/RAG - Ingestion and Indexing Pipeline]] — ingestion สร้าง chunks, embeddings, metadata, และ index
- [[02 AI Systems/RAG/Retrieval/RAG - Hybrid Retrieval]] — ผสาน vector retrieval กับ keyword retrieval
- [[02 AI Systems/RAG/Retrieval/RAG - Metadata Filtering and Permission-Aware Retrieval]] — metadata schema, filters, และ permission boundaries
