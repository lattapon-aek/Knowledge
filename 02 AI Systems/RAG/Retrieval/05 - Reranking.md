...
reranking เป็นชั้นคัดเอกสารรอบสองเพื่อเพิ่ม precision ก่อนประกอบ context เหมาะเมื่อ recall ดีแล้วแต่ top results ยัง noisy
...
retrieval มักถูกออกแบบให้ “หาให้เจอให้กว้างพอ” ส่วน reranking คือชั้นจัดอันดับรอบสองที่รับ candidate set มา reorder ให้ตรง intent มากขึ้น
...
เน้น better ordering, intent alignment, และ precision ของ top results  
...
Azure semantic ranker ใช้ language understanding models เพื่อ rescoring results และ promote results ที่ semantic ใกล้ intent มากกว่า
...
ในเชิงสถาปัตย์ reranking หลายแบบมีแนวคิดคล้าย cross-encoder: อ่าน query และ document pair ร่วมกัน แล้วประเมิน relevance แบบ pairwise หรือ scoring-based  
ถึงแม้ docs ของ Microsoft จะ framing เป็น semantic ranking feature มากกว่า model architecture detail แต่หลักคิดสำคัญคือ reranker มอง query + candidate ละเอียดกว่า first-stage retrieval จึงได้ ranking quality ดีขึ้นแต่แลก latency สูงขึ้น
