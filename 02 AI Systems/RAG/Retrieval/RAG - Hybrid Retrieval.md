...
type: note
status: evergreen
source: "Microsoft Learn (Azure AI Search Hybrid Search) · OpenAI Retrieval Docs · RAG-Anything (arxiv 2510.12323)"
...
OpenAI Retrieval docs ช่วยย้ำฝั่ง semantic retrieval ว่าควรคิดเรื่อง metadata filters และ ranking options ตั้งแต่ต้น
...
นอกจาก keyword + vector ที่เป็น hybrid retrieval แบบพื้นฐาน ยังมี hybrid อีกแบบคือ **vector similarity + graph traversal**

```mermaid
flowchart LR
    Q[User Query] --> VS[Vector Similarity Search]
    Q --> GT[Graph Traversal]
    VS --> F[Fusion]
    GT --> F
    F --> MR[Modality-Aware Ranking]
    MR --> CA[Context Assembly]
```

### ต่างจาก Keyword + Vector อย่างไร

| Hybrid แบบ | Signal ที่ 1 | Signal ที่ 2 | จุดแข็ง |
|---|---|---|---|
| Keyword + Vector | exact string matching | semantic similarity | ครอบคลุมทั้ง exact และ semantic queries |
| **Vector + Graph** | **semantic similarity** | **structural relationships** | **ครอบคลุมทั้ง semantic และ relationship-aware queries** |

### เมื่อไร Vector-Graph Fusion คุ้ม

- corpus มี entities และ relationships ที่ชัดเจน (เช่น research papers ที่อ้างอิงกัน)
- คำถามต้องการ multi-hop reasoning ข้าม entities
- ต้องการ relational coherence — retrieved elements ต้องเชื่อมกันอย่างมีความหมาย
- มี knowledge graph อยู่แล้วจาก ingestion phase

### Modality-Aware Ranking

เมื่อ corpus มีหลาย modalities ranking ต้องคำนึงถึง content type:
- ปรับ scoring ตาม query-specific modality preferences (เช่น คำถามเกี่ยวกับ performance → ให้น้ำหนัก tables มากขึ้น)
- รักษา semantic และ structural relationships ระหว่าง retrieved elements

→ ดูเพิ่มที่ [[02 AI Systems/RAG/Retrieval/RAG - Multimodal RAG|Multimodal RAG]] สำหรับ pipeline เต็ม
→ ดูเพิ่มที่ [[02 AI Systems/RAG/Retrieval/RAG - Knowledge Graph RAG|Knowledge Graph RAG]] สำหรับ graph retrieval พื้นฐาน
...
- [[02 AI Systems/RAG/Retrieval/RAG - Multi-Source Retrieval]] — hybrid retrieval เป็น pattern หนึ่งของ multi-source / multi-signal retrieval
