...
> เพิ่มจาก IBM Agentic Architecture

เมื่อตัดสินใจใช้ multi-agent แล้ว ต้องเลือก topology ที่เหมาะกับงาน:

```mermaid
flowchart LR
    A[Multi-Agent] --> V[Vertical]
    A --> H[Horizontal]
    A --> HY[Hybrid]
```

| Topology | โครงสร้าง | จุดแข็ง | จุดอ่อน | เหมาะกับ |
|---|---|---|---|---|
| **Vertical** | leader agent ดูแล subtasks, agents report กลับ | task efficiency, clear accountability | bottleneck ที่ leader, single point of failure | workflow automation, document generation |
| **Horizontal** | peer collaboration, decentralized decisions | dynamic problem solving, parallel processing | coordination challenges, slower decisions | brainstorming, complex interdisciplinary problems |
| **Hybrid** | leadership shifts ตาม task phase | versatile, adaptable | complexity, resource demanding | strategic planning, tasks ที่ต้องทั้ง structure และ creativity |

### เลือก Topology อย่างไร

- ถ้างานมี sequential steps ชัด → **Vertical** (เหมือน Subagents ใน Claude Code)
- ถ้างานต้อง parallel exploration → **Horizontal** (เหมือน Agent Teams ใน Claude Code)
- ถ้างานมีหลาย phases ที่ต้องการ leadership ต่างกัน → **Hybrid**

→ ดูตัวอย่าง implementation ที่ [[03 Tools/Claude Code/Core/03 - Orchestrator Pattern|Orchestrator Pattern]] (Subagents = vertical, Agent Teams = horizontal)
